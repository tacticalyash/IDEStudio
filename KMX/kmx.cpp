...
KMXTag::~KMXTag()
{
    for(KMXTag *tag : m_children_tags)
        delete tag;
}
...
struct Token
{
    ...
    enum : uint16_t{
        ...
        Property_Name = 0x08,            // <tag name = value></tag>
        Property_Value = 0x10,           // <tag name = value/>
        Equal_Sign = 0x20,               // =
...
static bool is_property_name(SConstForwardIterator begin_, SConstForwardIterator end_, SConstForwardIteratorRef search_end,TokenRef token)
{
    if(!isWord(*begin_))
        return false;
    token.str.push_back(*(begin_++));
    ...
    search_end = begin_;
    token.type = Token::Property_Name;
    return true;
};

static bool is_property_value(SConstForwardIterator begin_, SConstForwardIterator end_, SConstForwardIteratorRef search_end,TokenRef token)
{
    if(*begin_ != '\"')
        return false;
    ++begin_;
    ...
    if(begin_ == end_ || *begin_ == '\n')
        return false;
    token.type = Token::Property_Value;
    search_end = ++begin_;
    return true;
}

static bool is_equal_sign(SConstForwardIterator begin_, UNUSED SConstForwardIterator end_, SConstForwardIteratorRef search_end,TokenRef token)
{
    if(*begin_ != '=')
        return false;
    token.str = "=";
...
static bool is_greater_than_sign(SConstForwardIterator begin_, UNUSED SConstForwardIterator end_, SConstForwardIteratorRef search_end,TokenRef token)
{
    if(*begin_ != '>')
        return false;
    token.str = ">";
...
std::vector<Token> parse(std::string str, std::string &error)
{
    ...
    bool (*token_scanners[])(SConstForwardIterator, SConstForwardIterator, SConstForwardIteratorRef , Token &) = {
            ...
            is_closing_tag,         // </tag>
            is_closing_leaf_tag,    // />
            is_property_name,       // name
            is_property_value,      // "value"
            is_equal_sign,          // =
    ...
    while(begin_ != end_)
    {
        ...
        for(unsigned int i = 0; i < tag_count && !tag_found; ++i){token.str.clear();tag_found = token_scanners[i](begin_,end_,search_ended_at, token);}
        if(!tag_found)
        {
...
KMXTag *toKMXTag(const std::string &str, std::string &error)
{
    ...
    tokens.push_back({.str = "",.type = Token::Invalid_Token});

    bool good_token = true;
    KMXProperty property;
    std::size_t i;
    for(i = 1; i < size && good_token; ++i)
    {
        ...
        switch(tokens[i].type)
        {
            case Token::Opening_Tag:
                ...
                stack.push(temp);
                good_token = tokens[i+1].type & (Token::Closing_Leaf_Tag | Token::Property_Name | Token::Greater_Than_Sign);
                break;
            ...
            case Token::Property_Name:
                property.name = tokens[i].str;
                good_token = tokens[i+1].type & (Token::Equal_Sign);
                break;
            case Token::Property_Value:
                property.value = tokens[i].str;
                stack.top()->insertProperty(property);
                good_token = tokens[i+1].type & (Token::Property_Name | Token::Closing_Leaf_Tag | Token::Greater_Than_Sign);
                break;
            case Token::Equal_Sign:
                good_token = tokens[i+1].type & (Token::Property_Value);
                break;
